...
public interface CustomerWishlistRepository extends JpaRepository<CustomerWishlist, Integer> {
	...
	List<CustomerWishlist> findByCustomerCustomerId(int customerId);
	Void deleteByWishlistId(int wishlistId);
	Optional<CustomerWishlist> findByCustomerCustomerIdAndMfSchemaId(int customerId,int mfSchemaId);
}
